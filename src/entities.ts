...
export function makePlayer(k: KaboomCtx, posX: number, posY: number) {
    ...
    player.onCollide("enemy", async (enemy: GameObj) => {
        ...
        player.hurt();
        // Await is used here to make the call synchronous
        await k.tween(
            player.opacity,
            0,
            0.05,
...
export function setControls(k: KaboomCtx, player: PlayerGameObj) {
    // Kaboom gets all the things with tag xyz in any array, and as we have only one with inhaleEffect tag we are using 0.
    ...
    k.onKeyRelease((key) => {
        switch (key) {
            case "z":
                // If player is full then it has the feature to make a shooting star.
                if (player.isFull) {
                    player.play("kirbInhaling");
